...
if sites_csv.exists():
    try:
        ...
        st.warning(f"Could not read {sites_csv.name}: {exc}")
    else:
        st.success(f"Loaded {len(sites):,} flattened oxidation records.")
        st.dataframe(sites, use_container_width=True, hide_index=True)
else:
    st.info("No oxidation_sites.csv found yet. Save the configuration and run the oxidation stage.")

available = [path for path in (results_json, comparison_json, followup_json) if path.exists()]
if available:
    with st.expander("JSON result files", expanded=False):
