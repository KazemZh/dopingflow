...
def filter_cmd(
    config: Path = typer.Option(Path("input.toml"), "-c", "--config", exists=True),
    only: Optional[str] = typer.Option(None, "--only", help="Run only one folder name (e.g. Sb5_Zr5)"),
    force: bool = typer.Option(False, "--force", help="Rerun even if outputs already exist"),
    window_meV: Optional[float] = typer.Option(None, "--window-mev", help="Override window filter (forces mode=window)"),
    topn: Optional[int] = typer.Option(None, "--topn", help="Override top-N filter (forces mode=topn)"),
    verbose: bool = typer.Option(False, "--verbose", help="More detailed logs"),
) -> None:
...
def collect_cmd(
    ...
    _init(config, verbose)
    out_path = run_collect_from_toml(config)
